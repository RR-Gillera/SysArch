namespace MyWinFormAppFirst
{
    public partial class Form1 : Form
    {
        ...
        private void LoadStudents()
        {
            try
            {
                dataGridViewStudents.DataSource = dbHelper.GetAllStudents();
            }
            catch (Exception ex)
            {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            if (ValidateInput())
            {
                try
                {
                    dbHelper.AddStudent(txtName.Text, int.Parse(txtAge.Text), txtEmail.Text);
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (ValidateInput())
            {
                try
                {
                    dbHelper.UpdateStudent(int.Parse(txtId.Text), txtName.Text, int.Parse(txtAge.Text), txtEmail.Text);
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                try
                {
                    dbHelper.DeleteStudent(int.Parse(txtId.Text));
